...
class Algorithm:
    ...
    `model`: 
        ...
        object with a single, sigmoid-activated output node (rhyhtmnblues 
    ...
    def __init__(self, model, feature_extractors, used_features=None):
        ...
        `model`: 
            Underlying classification model. Can be a trained `torch.nn.Module` 
            object with a single, sigmoid-activated output node (rhythmnblues 
    ...
    def _fit_disabled(self, data):
        ...
        raise AttributeError(
            ...
            "Please use rhyhthmnblues.train module to fit neural networks."
        )
